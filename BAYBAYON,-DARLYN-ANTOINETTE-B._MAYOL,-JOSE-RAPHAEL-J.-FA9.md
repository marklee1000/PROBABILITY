...
$$f_x(x) = {n \choose x}{p^x}{(1-p)^{n-x}}$$, where x is the number of
successes
...
$$ \lim_{n \to \infty}{n \choose k} = \frac{n^k}{k!} \
\&\
...
have established the valididty of the Poisson approximation to the
